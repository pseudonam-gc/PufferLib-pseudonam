...
def collect_obs_stats(num_envs=256, steps=3000, buy_prob=0.1, seed=0):
    ...
    def std_var(samples):
        ...
        std = np.where(std < 0.02, 1.0, std)
        var = std ** 2  # derive from the (possibly guarded) std, not independently
        return std.astype(np.float32), var.astype(np.float32)

    owned_std, owned_var = std_var(owned_samples)
    market_std, market_var = std_var(market_samples)
    global_std, global_var = std_var(global_samples)

    return {
        'owned_std': owned_std, 'owned_var': owned_var,
        'market_std': market_std, 'market_var': market_var,
        'global_std': global_std, 'global_var': global_var,
    }
