...
typedef struct ThreesNet ThreesNet;
...
ThreesNet* make_threesnet(Weights* weights, int input_dim, int hidden_dim) {
    ThreesNet* net = calloc(1, sizeof(ThreesNet));
...
void free_threesnet(ThreesNet* net) {
    free(net->obs);
    free(net->layer1);
    free(net->gelu1);
    free(net->layer2);
...
void forward_threesnet(ThreesNet* net, unsigned char* observations, int* actions) {
